...
func TestKeyGeneration(t *testing.T) {
	...
	if bits := priv.N.BitLen(); int(bits) != size {
...
func TestGnuTLSKey(t *testing.T) {
	...
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase10Mod("290684273230919398108010081414538931343"),
			E: 65537,
		},
...
func fromBase10(base10 string) *safenum.Nat {
...
func fromBase10Mod(base10 string) *safenum.Modulus {
	return safenum.ModulusFromNat(*fromBase10(base10))
}
...
func init() {
	test2048Key = &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase10Mod(""),
			E: 3,
		},
...
func Benchmark3PrimeRSA2048Decrypt(b *testing.B) {
	...
	priv := &PrivateKey{
		PublicKey: PublicKey{
			N: fromBase10Mod(""),
			E: 3,
		},
...
func TestEncryptOAEP(t *testing.T) {
	...
	for i, test := range testEncryptOAEPData {
		...
		public := PublicKey{safenum.ModulusFromBytes(n.Bytes()), test.e}
...
func TestDecryptOAEP(t *testing.T) {
	...
	for i, test := range testEncryptOAEPData {
		...
		private.PublicKey = PublicKey{safenum.ModulusFromBytes(n.Bytes()), test.e}
